...
GPIO.setup(ct1_channel,GPIO.IN)
GPIO.setup(ct2_channel,GPIO.IN)
GPIO.setup(ct3_channel,GPIO.IN)
GPIO.setup(ct4_channel,GPIO.IN)
GPIO.setup(ct5_channel,GPIO.IN)
GPIO.setup(ct6_channel,GPIO.IN)
...
def readGPIO(GPIOnum)
    data=GPIO.input(GPIOnum)
    return data
...
def collect_data(numSamples):
    ...
    for _ in range(numSamples):
        ct1 = readGPIO(ct1_channel)
        ct5 = readGPIO(ct5_channel)
        ct2 = readGPIO(ct2_channel)
        v = readGPIO(v_sensor_channel)
        ct3 = readGPIO(ct3_channel)
        ct4 = readGPIO(ct4_channel)
        ct6 = readGPIO(ct6_channel)
